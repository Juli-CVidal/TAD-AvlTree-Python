...
"""Implementación ejercicio 7"""
def isMinor(node,key): #log M
	if (node == None):
		return True
	if (node.key<key):
		return False
	else:
		return isMinor(node.leftNode,key)


def isMajor(node,key): #log N
	if (node == None):
		return True
	if (node.key>key):
		return False
	else:
		return isMajor(node.rightNode,key)

def combineTrees(A,B,nodeX):
	if (not isMinor(B.root,nodeX.key) and not isMajor(A.root,nodeX.key)): #logN+logM
		return None
	newAvlTree = AVLTree()
	newAvlTree.root = nodeX
	nodeX.leftNode = A.root
	nodeX.rightNode = B.root

	return newAvlTree


"""Implementación ejercicio 1 parte 3"""
import math
def heightWithAmount(n):
	return math.log10(n+1) - 1

def heightWithRoot(rootNode):
	if (rootNode == None):
		return 0

	if (rootNode.balanceFactor > 0):
		return 1 + heightWithRoot(rootNode.leftNode) 
	else:
		return 1 + heightWithRoot(rootNode.rightNode)


"""Implementación ejercicio 2 parte 3"""
def countNodes(node,leftLimit,rightLimit):
	if (node == None):
		return 0

	if (node.key >= leftLimit and node.key <= rightLimit):
		return 1 + (countNodes(node.leftNode,leftLimit,rightLimit) + countNodes(node.rightNode,leftLimit,rightLimit))
		
	elif (node.key < leftLimit):
		return countNodes(node.rightNode,leftLimit,rightLimit)
	else:
		return countNodes(node.leftNode,leftLimit,rightLimit)